...
public class JwDBManager implements DBManager {
    private static final Logger LOGGER = LoggerFactory.getLogger(JwDBManager.class);
    ...
    public DB getDefaultDB() {
        return dbPools.values().iterator().next();
    }

    public DB getDB(String dbName) {
        return StringUtils.isEmpty(dbName) ? getDefaultDB() : dbPools.get(dbName);
    }

    public Collection<String> getDBNames() {
        return Collections.unmodifiableSet(dbPools.keySet());
    }
